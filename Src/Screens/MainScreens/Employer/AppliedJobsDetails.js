...
const AppliedJobsDetails = (props) => {
    ...
    useEffect(() => {
        sendJobView()
        // alert(JSON.stringify(props.route.params?.JobDetails.CHANNEL_NAME))
        // alert(JSON.stringify(props.route.params?.JobDetails.APP_ID))
        // alert(JSON.stringify(props.route.params?.JobDetails.INTERVIEW_TIME))
        // alert(JSON.stringify(props.route.params?.JobDetails.INTERVIEW_DATE))
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={{ marginLeft: wp(10), marginRight: wp(3) }}>
                <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: UserRole == 'employer' ? 10 : 0, alignItems: "center" }}>
                    <View style={{ flex: 0.65 }}>
                        <Text style={{ color: Colors.BlueColor, fontWeight: "bold" }}>POSITION</Text>
                        <Text style={{ marginTop: 5 }}>{props.route.params?.JobDetails?.TITLE?.toUpperCase()}</Text>
                    </View>
                    {UserRole != 'employer' &&
                        <Button title={"Join Interview"} style={{ flex: 0.35 }}
                            onPress={() => props.navigation.navigate("VideoCall", {
                                appId: props.route.params?.JobDetails.APP_ID,
                                channelName: props.route.params?.JobDetails.CHANNEL_NAME
                            })}
                        />}
                </View>
                <Text style={{ color: Colors.BlueColor, marginTop: 15, fontWeight: "bold" }}>CATEGORY</Text>
                ...
                <Text style={{ color: Colors.BlueColor, marginTop: 15, fontWeight: "bold" }}>DESCRIPTION</Text>
                <Text style={{ marginTop: 5, textAlign: "justify" }}>{props.route.params?.JobDetails?.DESCRIPTION}</Text>
                <Text style={{ color: Colors.BlueColor, marginTop: 15, fontWeight: "bold" }}>INTERVIEW STATUS</Text>
                <Text style={{ marginTop: 5, textAlign: "justify" }}>{props.route.params?.JobDetails?.INTERVIEW_STATUS}</Text>
                <Text style={{ color: Colors.BlueColor, marginTop: 15, fontWeight: "bold" }}>DATE AND TIME</Text>
                <Text style={{ marginTop: 5, textAlign: "justify" }}>{`${props.route.params?.JobDetails?.INTERVIEW_DATE} ${props.route.params?.JobDetails?.INTERVIEW_TIME}`}</Text>
            </View>
