...
const MyJobs = (props) => {
    ...
    const readJobs = async () => {
        let data = {}
        data["id"] = userId;
        const res = await Axios(UserRole == 'employer' ? "read_self_jobs.php" : "read_jobs.php", data);
        if (res.status == 3) {
            setNoJob(true)
        } else {
    ...
    return (
        <View style={{ flex: 1 }}>
            <Header_ title={"My Jobs"}
                ...
                type={ UserRole == 'employer' ? "Feather" : "Ionicons"} name={UserRole == 'employer' ? "plus" : "briefcase"}
                onLeftPress={() => props.navigation.navigate( UserRole == 'employer' ? "PostJob" : "AppliedJobs")} />
